...
def main():
    ...
    if args.quick_run.lower() == "false":
        TOP_N = 10
    train_dataset, val_dataset, test_dataset = SwissSummarizationDataset(
        top_n=TOP_N, validation=True
    )
    ...
    processor = BertSumAbsProcessor(
        model_name=MODEL_NAME, cache_dir=args.cache_dir, max_src_len=args.max_pos_length
    )
    summarizer = BertSumAbs(
        processor, model_name=MODEL_NAME, cache_dir=args.cache_dir, max_pos_length=args.max_pos_length
    )
    mp.spawn(
        main_worker,
        nprocs=ngpus_per_node,
        args=(ngpus_per_node, summarizer, train_dataset, val_dataset, test_dataset, args),
    )


def main_worker(
    local_rank, ngpus_per_node, summarizer, train_dataset, val_dataset, test_dataset, args
):
    ...
    def this_validate(class_obj):
        return validate(class_obj, val_dataset)
